//! Library face of the `marila` crate.
//!
//! `main.rs` is a thin shim that reads env, calls [`build_router`], and
//! `axum::serve`s the result. Tests (and any future embedders) can
//! bypass the env layer and call [`build_router`] directly with a
//! [`ServerConfig`] built by hand.

use std::sync::Arc;

use anyhow::{Context, Result};
use axum::{Router, routing::get};
use marila_core::DuckDbStateStore;
use marila_storage::{S3BucketStore, S3Config};
use marila_tables::{
    AppState as TablesAppState, DEFAULT_LAKEKEEPER_URL, LakekeeperClient, LakekeeperConfig,
};
use marila_vectors::AppState as VectorsAppState;
use tower_http::trace::{DefaultMakeSpan, TraceLayer};
use tracing::info;

/// All knobs the running marila server needs.
///
/// Construct via [`ServerConfig::from_env`] (for `main.rs`) or
/// [`ServerConfig::for_tests`] (for the embedded harness).
#[derive(Clone, Debug)]
pub struct ServerConfig {
    /// Address axum binds to. Tests pass `127.0.0.1:0` and read the
    /// actual port off the bound listener.
    pub bind_addr: String,
    pub s3_endpoint: String,
    pub s3_access_key_id: String,
    pub s3_secret_access_key: String,
    pub s3_region: String,
    pub account_id: String,
    pub state_db: String,
    /// Lakekeeper management + catalog base URL (e.g. `http://localhost:8181`).
    pub lakekeeper_url: String,
    /// S3 endpoint Lakekeeper warehouses should write to — usually the
    /// docker-network alias `http://rustfs:9000` so Lakekeeper can reach
    /// RustFS from inside the compose graph (see DISCOVERIES D-2).
    pub lakekeeper_storage_endpoint: String,
}

impl ServerConfig {
    /// Read every field from environment variables, falling back to
    /// production defaults. Used by `main.rs`.
    pub fn from_env() -> Result<Self> {
        Ok(Self {
            bind_addr: env_or("MARILA_BIND_ADDR", "0.0.0.0:8080"),
            s3_endpoint: env_or("MARILA_S3_ENDPOINT", "http://localhost:9000"),
            s3_access_key_id: env_or("MARILA_S3_ACCESS_KEY_ID", "marila"),
            s3_secret_access_key: env_or("MARILA_S3_SECRET_ACCESS_KEY", "marilasecret"),
            s3_region: env_or("MARILA_S3_REGION", "eu-west-1"),
            account_id: env_or("MARILA_AWS_ACCOUNT_ID", "000000000000"),
            // Filename intentionally not `state.duckdb` — DuckDB names
            // the catalog after the file stem, which would then collide
            // with our `state` schema and make `state.vector_buckets`
            // ambiguous (CLAUDE.md C-10).
            state_db: env_or("MARILA_STATE_DB", "data/marila.duckdb"),
            lakekeeper_url: env_or("MARILA_LAKEKEEPER_URL", DEFAULT_LAKEKEEPER_URL),
            lakekeeper_storage_endpoint: env_or(
                "MARILA_LAKEKEEPER_STORAGE_ENDPOINT",
                "http://rustfs:9000",
            ),
        })
    }

    /// Sensible defaults for an in-process embedding (typically integration
    /// tests). `s3_endpoint` and `state_db` come from the caller because
    /// they have to be unique-per-process to avoid collisions.
    pub fn for_tests(s3_endpoint: impl Into<String>, state_db: impl Into<String>) -> Self {
        Self {
            bind_addr: "127.0.0.1:0".into(),
            s3_endpoint: s3_endpoint.into(),
            s3_access_key_id: "marila".into(),
            s3_secret_access_key: "marilasecret".into(),
            s3_region: "eu-west-1".into(),
            account_id: "000000000000".into(),
            state_db: state_db.into(),
            lakekeeper_url: DEFAULT_LAKEKEEPER_URL.into(),
            // Tests don't exercise the Lakekeeper-side iceberg writes,
            // so this can stay at the docker-alias default.
            lakekeeper_storage_endpoint: "http://rustfs:9000".into(),
        }
    }
}

/// Construct the full axum router (vectors + tables + /health), wired to
/// the storage and state stores described by `cfg`. Doesn't bind; the
/// caller picks how to `axum::serve` it (or `tokio::spawn` it).
pub async fn build_router(cfg: ServerConfig) -> Result<Router> {
    let state =
        Arc::new(DuckDbStateStore::open(&cfg.state_db).context("open state db")?);
    let storage = Arc::new(
        S3BucketStore::connect(S3Config {
            endpoint: cfg.s3_endpoint.clone(),
            access_key_id: cfg.s3_access_key_id.clone(),
            secret_access_key: cfg.s3_secret_access_key.clone(),
            region: cfg.s3_region.clone(),
        })
        .await
        .context("connect to s3 backend")?,
    );

    // Rehydrate every known (bucket, index) backing table from its
    // RustFS snapshots — RustFS is the source of truth (FV-4). Boot
    // continues if a single index fails; the warn-level logs make
    // diagnostics easy.
    match marila_vectors::rehydrate_from_snapshots(&*state, &*storage).await {
        Ok(n) if n > 0 => info!(restored = n, "rehydrated vector snapshots into DuckDB"),
        Ok(_) => {}
        Err(e) => tracing::warn!(error = %format!("{e:#}"), "rehydrate pass failed"),
    }

    let vectors_state = VectorsAppState {
        state: state.clone(),
        storage: storage.clone(),
        region: cfg.s3_region.clone(),
        account_id: cfg.account_id.clone(),
    };

    let lakekeeper = Arc::new(LakekeeperClient::new(LakekeeperConfig {
        base_url: cfg.lakekeeper_url.clone(),
        storage_endpoint: cfg.lakekeeper_storage_endpoint.clone(),
        storage_access_key_id: cfg.s3_access_key_id.clone(),
        storage_secret_access_key: cfg.s3_secret_access_key.clone(),
        storage_region: cfg.s3_region.clone(),
    }));
    let tables_state = TablesAppState {
        state: state.clone(),
        storage: storage.clone(),
        lakekeeper,
        region: cfg.s3_region.clone(),
        account_id: cfg.account_id.clone(),
    };

    Ok(Router::new()
        .route("/health", get(health))
        .merge(marila_vectors::router(vectors_state))
        .merge(marila_tables::router(tables_state))
        .layer(
            TraceLayer::new_for_http()
                .make_span_with(DefaultMakeSpan::new().include_headers(true)),
        ))
}

async fn health() -> axum::Json<serde_json::Value> {
    axum::Json(serde_json::json!({"status": "ok"}))
}

fn env_or(key: &str, default: &str) -> String {
    std::env::var(key).unwrap_or_else(|_| default.to_owned())
}
